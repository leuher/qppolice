    ...
    var Table = {
        ...
        api: {
            ...
            bindevent: function (table) {
                ...
                $(toolbar).on('click', Table.config.sendBtn, function () {
                    var that = this;
                    var ids = Table.api.selectedids(table);
                    var url = options.extend.send_url;
                    var id = window.location.href.match(/\/(\d+)\?/)[1];
                    console.log(ids)
                    console.log(url)
                    //Table.api.multi(url, ids, table, that);
                    //
                    console.log(id)
                    window.location.href = "/admin/"+url + '?message_id='+ id +'&ids='+ ids;
